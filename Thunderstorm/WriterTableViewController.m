...
- (id)init
{
    ...
    if(self){
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0.0, 320.0, 568.0) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 0){
        ...
        [cell.titleTextField setText:[self.timelineData objectForKey:@"title"]];
        ...
        cell.titleTextField.inputAccessoryView = inputAccessory;
...
- (void)addNewCell:(id)sender
{
    ...
    NSIndexPath *path = [NSIndexPath indexPathForRow:tweetData.count - 1 inSection:1];
...
{
    if(textField.tag == 0){
        [self.timelineData setObject:textField.text forKey:@"title"];
    }
}

- (void)textViewDidChange:(UITextView *)tv
{
    // descriptionTextView in title is a separately handled textview
    if(tv.tag == -1){
        WriterDescriptionTableViewCell *cell = (WriterDescriptionTableViewCell *)[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
        if([tv.text isEqual:@""]){
            cell.descriptionPlaceholder.hidden = NO;
        } else {
            cell.descriptionPlaceholder.hidden = YES;
        }
        [self.timelineData setObject:tv.text forKey:@"description"];
    } else {
        ...
        if(calcNumberOfLines != prevNumberOfLines.intValue){
            ...
            [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:tv.tag inSection:1] atScrollPosition:UITableViewScrollPositionTop animated:YES];
        }
    }
}
